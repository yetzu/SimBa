...
def find_latest_ckpt(save_dir: str) -> str:
    ...
    # Recursive search for checkpoints in subdirectories
...
def main():
    ...
    # 2. Data
    # Note: ScwdsDataModule handles inference set setup
    data_module = ScwdsDataModule(
        data_path=config.data_path,
        batch_size=1,
        num_workers=1
    )
    # Ensure setup is called for infer stage
    ...
    with torch.no_grad():
        for bidx, (metadata_list, batch_x, input_mask) in enumerate(infer_loader):
            try:
                ...
                input_mask = input_mask.to(device) # Mask is also needed if model uses it
                
                # Inference
                # It expects a batch tuple: (metadata, x, input_mask)
                batch_y = model.infer_step((metadata_list, batch_x, input_mask), batch_idx=bidx)
                
                # Output shape is [B, T, C, H, W], we take first item in batch, channel 0
                ...
                # 可视化
                if args.vis:
                    # Mamba input has 54 channels, channel 0 is typically radar
